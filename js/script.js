chrome.contextMenus.create({
	title: "Experience Colour Blindness",
	contexts: ["all"]
});
...
chrome.contextMenus.onClicked.addListener(function onClickHandler(info, tab) {
	...
	var default_svg_data = '' +
		'<svg id="colorblind-filters" width="0" height="0"><defs>' +
	...
	var svg_data_file = "http://altreus.github.io/colourblind/colourblind.svg";
	if (localStorage["svg_data_file"]) {
		svg_data_file = localStorage["svg_data_file"];
	}

	var xhr = new XMLHttpRequest();
	xhr.open("GET", svg_data_file, true);
